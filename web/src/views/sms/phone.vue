<template>
  <div class="app-container">
    <div class="header">
      <span>
        <el-button type="primary" size="mini" @click="schedulerAdd">导入电话号码</el-button>
    ...
    >
      ...
      <el-table-column label="归属设备" align="center">
        <template slot-scope="scope">
          ...
          <el-button type="text" @click="chdevice(scope.row)">更改</el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" prop="create_time" label="结果">
        <template slot-scope="scope">
          <span v-if="scope.row.status == '发送中' && !scope.row.update_time ">结果未上传</span>
          <span v-if="scope.row.update_time ">已发送，{{ scope.row.update_time | time }}</span>
        </template>
      </el-table-column>
...
import EmailAdd from "./phone-add";
...
export default {
  ...
  methods: {
    ...
    chdevice(row) {
      this.$prompt("请输入新的归属", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消"
      }).then(({ value }) => {
        this.listLoading = true;
        request({
          url: "/sms/phone/device",
          method: "post",
          data: { id: row.id, device: value }
        }).then(res => {
          this.fetchData();
        }).finally(() => {
          this.listLoading = false;
